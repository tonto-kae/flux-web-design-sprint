...
import { newsSectionQuery, type SanityImage } from "@/sanity/lib/queries";
...
export default async function News() {
  const data = await client.fetch<Data>(newsSectionQuery, {}, { next: { revalidate: 60 } });
  if (!data) return null;
      ...
      <div className="lg:hidden flex flex-col gap-8">
        ...
        <div className="overflow-x-auto -mx-px scroll-smooth snap-x snap-mandatory">
          <div className="flex gap-4 px-4 w-max">
            {items.map((item) => (
              <div key={item._id} className="w-[300px] shrink-0 snap-start">
          ...
          {items.map((item, idx) => {
            ...
            return (
              <Fragment key={item._id}>
                <div className={`w-[353px] shrink-0 ${isMiddle ? "pt-[120px]" : ""}`}>
                  <NewsCard item={item} aspect="aspect-[353/469]" />
                </div>
                {idx < items.length - 1 && (
